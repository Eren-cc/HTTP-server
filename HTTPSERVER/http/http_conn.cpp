...
//epoll注册，ET模式.事件重置为EPOLLONESHOT
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        {
            //解析请求行 获取 m_url，m_version，m_method
        ...
        {
    /*4.10*///解析请求头 获取  m_linger，m_content_length（请求主体长度），m_host
...
//针对get请求的数据处理
http_conn::HTTP_CODE http_conn::do_request()//4.10
{
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    case FILE_REQUEST://文件存在，200
    {
        ...
        {
            add_headers(m_file_stat.st_size);//第一个iovec指针指向响应报文缓冲区，长度指向m_write_idx
...
void http_conn::process()
{
    ...
    {
        modfd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);//（init初始化就有epollshot）注册epollin，开启epollshot，主线程检测到epoll开始read
        return;//等待再次触发socket读取事件
    }
    bool write_ret = process_write(read_ret);//4.17
    if (!write_ret)
    {
        close_conn();
    }
    modfd(m_epollfd, m_sockfd, EPOLLOUT, m_TRIGMode);//注册epollout，开启epollshot，主线程检测到epollout开始write
}
